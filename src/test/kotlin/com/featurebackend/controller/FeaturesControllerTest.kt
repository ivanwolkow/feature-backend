...
class FeaturesControllerTest {
    ...
    @Test
    fun `should return a quicklook data for a feature`() {
        val expectedContent = ClassPathResource("/quicklook-content.bin").file.readBytes()
        val body = given()
        ...
        assertArrayEquals(expectedContent, body)
    }

    @Test
    fun `should return an error when feature not found`() {
        given()
            .get("/features/wrong-uuid/quicklook")
            .then()
            .statusCode(equalTo(HttpStatus.NOT_FOUND.value()))
            .body("message", equalTo("Feature not found"))
    }
}
